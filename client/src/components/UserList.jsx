...
//fetching the users from dummy user data
const UserList = ({ setTeam, team}) => {
    ...
    //if no users are selected, show user number [0]
    useEffect(() => {
      if(team?.length < 1) {
...
<ListboxButton className='relative w-full cursor-default rounded bg-white pl-3 pr-10 text-left px-3 py-2.5 2xl:py-3 border border-gray-300 sm:text-sm'>
            <span className='block truncate'>
                {/* assign the one task to multiple users */}
                ...
                >
                  {({ selected }) => (
                    <>
                      ...
                      {/* if the selected is true, we wanna have a span tag that will show an icon otherwise, don't show anything*/}
                      {selected ? (
